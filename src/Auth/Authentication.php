...
abstract class Authentication {
	 ...
	 * Global SDK settings
	 *
	 * @var AbstractSettings Instance of AlexaCRM\CRMToolkit\Settings class
	 */
	public $settings;
	...
	public function __construct( $settings, $client ) {
		...
		$this->client->logger->debug( 'cURL version', [
	...
	protected function retrieveToken( $service ) {
		...
		$newToken->expiryTime = Client::parseTime( $expiryTime, 'Y-m-d\TH:i:s.v\Z' );
	...
	protected function getTokenCredentials() {
		...
		if ( $this->settings->authMethod === OnlineS2SSecretAuthenticationSettings::SETTINGS_TYPE ) {
			$tokenCredentials = array_merge( $tokenCredentials, [
